...
* Non-nil MSGP means echo the region size."
  ...
  (let* ((mrk1    (make-marker))
    ...
    (when msgp (message "%s region: %d to %d" (if (called-interactively-p) "Recorded" "Narrowed")
...
that is the value of `zz-izones-var' can be modified."
  ...
  (save-excursion
	...
	(setq beg  (point)))
      (goto-char end)
      (re-search-backward "^\n" (- (point) 1) t)
      (when (or (called-interactively-p)  zz-add-zone-anyway-p) (zz-add-zone beg end nil nil nil 'MSG))
      (narrow-to-region beg end))))
